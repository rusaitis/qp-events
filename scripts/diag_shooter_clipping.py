...
def _solve(method: str, *, freq_low: float, n_modes: int = 6) -> tuple[np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    log.info("  mode 1 = %.6f mHz   (Rusaitis 2021 ref = %.3f mHz)", f1_matrix, _REF_RUSAITIS_MHZ)
    ...
    log.info("=" * 72)
    log.info("Shooter sweep over freq_range[0]  (KMAG L=%.0f, freq_high=%.0e, resolution=%d)",
             _L_SHELL, _FREQ_HIGH, _RESOLUTION)
    log.info("  freq_low (rad/s)  |  shooter mode 1 (mHz)  |  Δ vs matrix (mHz)  |  n_modes")
    log.info("  " + "-" * 76)
    for freq_low in sweep:
        try:
            ...
            delta = f1 - f1_matrix
            n_m = len(fs)
            log.info("  %16.2e  |  %20.6f  |  %18.6f  |  %d",
                     freq_low, f1, delta, n_m)
            rows.append({"freq_low_rads": freq_low, "shooter_f1_mHz": f1,
                         "delta_vs_matrix_mHz": delta, "n_modes": n_m})
        except Exception as exc:
            log.warning("  shooter failed at freq_low=%.2e: %s", freq_low, exc)
            rows.append({"freq_low_rads": freq_low, "shooter_f1_mHz": float("nan"),
                         "delta_vs_matrix_mHz": float("nan"), "n_modes": 0})

    # CSV
    csv_path = out / "shooter_clipping_l8.csv"
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["freq_low_rads", "shooter_f1_mHz",
                                                "delta_vs_matrix_mHz", "n_modes"])
        writer.writeheader()
    ...
    ax.axhline(f1_matrix, ls="--", color="C2", lw=1.6,
               label=f"matrix mode 1 = {f1_matrix:.4f} mHz")
    ax.axhline(_REF_RUSAITIS_MHZ, ls=":", color="C3", lw=1.6,
    ...
    ax.set_title(f"KMAG L={_L_SHELL:.0f} noon — does the shooter clip the sub-WKB mode?")
    ...
    log.info("  shooter agreed with matrix (Δ<5e-3 mHz) for %d/%d settings",
             len(matches), len(valid))
    log.info("  shooter diverged (Δ≥5e-3 mHz) for %d/%d settings",
             len(clips), len(valid))
    if clips:
        clip_thresh = min(r["freq_low_rads"] for r in clips)
        clip_f1 = next(r["shooter_f1_mHz"] for r in clips if r["freq_low_rads"] == clip_thresh)
        log.info("  earliest divergence at freq_range[0] = %.2e rad/s "
                 "(shooter f1 = %.4f mHz)", clip_thresh, clip_f1)
        if 0.10 < clip_f1 < 0.14:
            log.info("  ⇒ clipping at this threshold yields ~0.12 mHz "
                     "(matches Rusaitis 2021).")
        else:
            log.info("  ⇒ post-clip f1 = %.4f mHz does not match Rusaitis "
                     "(0.12 mHz); clipping not the reconciliation.", clip_f1)
    else:
        log.info("  shooter agreed across the full sweep — no clipping observed.")
        log.info("  hypothesis (Rusaitis 2021 missed mode 1 via bracket-scan) "
