...
EXTENDED_BANDS: list[Band] = [
    Band(name="BELOW_QP30",
         period_min_sec=10 * _MIN, period_max_sec=20 * _MIN,
         period_centroid_sec=14 * _MIN),
    Band(name="QP30_E",
         period_min_sec=20 * _MIN, period_max_sec=40 * _MIN,
         period_centroid_sec=30 * _MIN),
    Band(name="QP60_E",
         period_min_sec=40 * _MIN, period_max_sec=80 * _MIN,
         period_centroid_sec=60 * _MIN),
    Band(name="QP120_E",
         period_min_sec=80 * _MIN, period_max_sec=150 * _MIN,
         period_centroid_sec=120 * _MIN),
    Band(name="ABOVE_QP120",
         period_min_sec=150 * _MIN, period_max_sec=360 * _MIN,
...
def main(argv: list[str] | None = None) -> int:
    ...
    for k, idx in enumerate(sampled):
        ...
        if (k + 1) % 20 == 0:
            log.info("  %d / %d segments done, %d events so far",
                     k + 1, len(sampled), len(all_events))

    log.info("sweep complete: %d events from %d segments",
             len(all_events), len(sampled))
    ...
    ax.hist(df["period_min"], bins=bins, color="#4dd2ff", alpha=0.85,
    ...
    for name, (lo, hi) in canonical_edges.items():
        ...
        ax.text((lo * hi) ** 0.5, ax.get_ylim()[1] * 0.95, name,
                color=band_color[name], ha="center", va="top",
                fontsize=9, fontweight="bold")
    # Gap shading.
    for lo, hi in ((40, 45), (80, 90)):
        ax.axvspan(lo, hi, alpha=0.40, facecolor="#444", zorder=0,
                   hatch="//", edgecolor="#888")
    # Above / below shading.
    ...
    ax.set_ylabel(f"events per bin  (N={len(df)}, "
                  f"{len(sampled)} segments)")
    ax.set_title(
        "Band-edge sensitivity sweep — extended scheme [10–360 min]"
    )
    ...
    for zone in ("below_QP30 [10,20)",
                 "QP30 [20,40)", "gap1 [40,45)",
                 "QP60 [45,80)", "gap2 [80,90)",
                 "QP120 [90,150)", "above_QP120 [150,360)"):
    ...
    ax.text(
        ...
        bbox=dict(facecolor="white", alpha=0.85,
                  edgecolor="#888", boxstyle="round,pad=0.4"),
    )
