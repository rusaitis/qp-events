...
def main():
    ...
    parser.add_argument(
        "--window-size", type=str, default="1920x1080", help="Window size WxH"
    )
    parser.add_argument(
        "--orbit-radius", type=float, default=0.10, help="Orbit tube radius (R_S)"
    )
    ...
    # Saturn sphere
    saturn = pv.Sphere(
        radius=1.0, center=(0, 0, 0), theta_resolution=60, phi_resolution=60
    )
    pl.add_mesh(
        saturn,
        color="#e8d282",
    ...
    ring = pv.Disc(
        center=(0, 0, 0), inner=1.24, outer=2.27, normal=(0, 0, 1), r_res=5, c_res=120
    )
    ...
    for v in ticks:
        pl.add_mesh(
            pv.Line((-lim, v, 0), (lim, v, 0)),
            color="white",
            opacity=0.12,
            line_width=1,
        )
        pl.add_mesh(
            pv.Line((v, -lim, 0), (v, lim, 0)),
            color="white",
            opacity=0.12,
            line_width=1,
        )
