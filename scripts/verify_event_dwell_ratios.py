...
def main() -> None:
    ...
    print(f"  date range           : {df['peak_time'].min()} to {df['peak_time'].max()}")
    print(f"  duration  (h)        : "
    ...
    if not dw_zarr.exists():
        print(f"  MISSING — skip ratio against the canonical 3D + dipole_inv_lat schemas")
    else:
        ...
        if "QP60_dipole_inv_lat_magnetosphere" in ev:
            ...
            print(f"  QP60 dipole_inv_lat ratio: max={r_max:.4f}, "
    ...
    if not eq_zarr.exists():
        print(f"  MISSING — skip ratio against the kmag_eq_r schema")
    else:
        eq = xr.open_zarr(eq_zarr)
        print(f"  variables            : {sorted(eq.data_vars)[:6]}…")
        print(f"  kmag_eq_r dwell (closed): "
        ...
        if has_eq_r and "QP60_kmag_eq_r_closed_total" in ev:
            ...
            print(f"  (KMAG schemas not populated in event-time zarr — "
                  f"re-run bin_events_round8.py --with-kmag-trace)")
