...
class TestBandpassTransverseRatio:
    ...
    def _make_alfvenic(
        self, n: int = 2160, period_min: float = 60.0, amplitude: float = 1.0
    ...
    def test_bandpass_ratio_near_one_for_off_band_noise(self):
        ...
        assert 0.2 < bp_tr < 6.0, f"band-pass ratio for noise {bp_tr:.2f} should be ~1"
...
class TestPolarizationComparison:
    ...
    def _wave(self, phase_offset_deg: float) -> tuple[np.ndarray, np.ndarray]:
        ...
        p2 = np.sin(2 * np.pi * t / self.PERIOD + np.radians(phase_offset_deg))
        return p1, p2
    ...
    def test_stokes_more_robust_than_xcorr_for_real_events(self):
        ...
        p1 = np.sin(2 * np.pi * t / self.PERIOD) + noise_level * rng.standard_normal(n)
        p2 = (
            np.sin(2 * np.pi * t / self.PERIOD)  # same phase → linear
