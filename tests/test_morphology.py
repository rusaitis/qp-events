...
def _gaussian_envelope_sine(
    n: int = N, period: float = PERIOD_SEC, dt: float = DT
) -> np.ndarray:
    t = np.arange(n) * dt
    centre = t[n // 2]
    width = n * dt / 4.0
...
def _chirped_sine(
    n: int = N,
    period_start: float = PERIOD_SEC,
    chirp_rate_hz_s: float = 1e-7,
    dt: float = DT,
) -> np.ndarray:
...
class TestHarmonicRatio:
    ...
    def test_zero_period_guard(self):
        ...
        ratio = harmonic_ratio(
            x, DT, 60.0
        )  # 1-minute period (just outside Nyquist range)
...
class TestEnvelopeRiseFall:
    def test_gaussian_envelope_symmetric(self):
        ...
        assert 0.3 < ratio < 3.0, (
            f"ratio={ratio:.2f} expected ~1 for symmetric envelope"
        )
...
class TestAmplitudeGrowthRate:
    ...
    def test_steady_sine_near_zero(self):
        ...
        assert abs(rate) < 2.0, (
            f"Steady sine should have ~0 growth rate, got {rate:.3f}"
        )
