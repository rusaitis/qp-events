...
# that imported these from qp.benchmark.runner (the canonical home is
# now qp.events.detector).
__all__ = [
    "BENCHMARK_DIR",
    "MAX_MVA_PARALLEL_FRACTION",
    "MIN_DEGREE_OF_POLARIZATION",
...
def _detect_events_in_dataset(
    ...
    """Thin wrapper around :func:`qp.events.detector.detect_round8`.

    Returns only the peaks (no diagnostic gate values) since the
    benchmark scorer doesn't use them. Real-data sweeps that need the
    gate values for persistence should call ``detect_round8`` directly.
    """
    return [d.peak for d in detect_round8(t, fields, dt)]
