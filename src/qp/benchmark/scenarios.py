...
def decoy_ppo_only() -> ScenarioConfig:
    return ScenarioConfig(
        ...
        event_specs=[
            EventSpec(
                band="QP60",
                period_sec_override=10.7 * 3600.0,
                amplitude=0.0,
                center_hours=120.0,
                decay_hours=100.0,
                should_detect=False,
...
def tier3_plasma_sheet() -> ScenarioConfig:
    ...
    centers = _centers(6, 10)
    bursts = [
        NoiseBurstSpec(center_hours=c, duration_hours=3.0, sigma_multiplier=5.0)
        for c in centers[:3]
    ]
    return ScenarioConfig(
...
def tier4_nonstationary() -> ScenarioConfig:
    ...
    bursts = [
        NoiseBurstSpec(
            center_hours=h, duration_hours=5.0, sigma_multiplier=3.0 + (h % 7)
        )
...
def decoy_roll_artifacts() -> ScenarioConfig:
    ...
    centers = _centers(6, 10)
    rolls = [
        RollArtifactSpec(center_hours=c, duration_hours=2.0, rotation_deg=15.0 + i * 5)
